...
function resize_vector_if_size_mismatch(subject, model)
    if #subject ~= #model then
        subject:resize(#model)
    end
end

function set_vector_to_data(vector, data)
    resize_vector_if_size_mismatch(vector, data)
    for i, imported_data in ipairs(data) do
        vector[imported_data.id] = imported_data.value
    end
end
...
function set_appearance(unit, appearance_data)
    ...
    set_vector_to_data(appearance.body_modifiers, appearance_data.body_modifiers)
    set_vector_to_data(appearance.bp_modifiers, appearance_data.bp_modifiers)
    appearance.size_modifier = appearance_data.size_modifier

    -- Tissue-related data
    set_vector_to_data(appearance.tissue_style, appearance_data.tissue_style)
    set_vector_to_data(appearance.tissue_style_civ_id, appearance_data.tissue_style_civ_id)
    set_vector_to_data(appearance.tissue_style_id, appearance_data.tissue_style_id)
    set_vector_to_data(appearance.tissue_style_type, appearance_data.tissue_style_type)
    set_vector_to_data(appearance.tissue_length, appearance_data.tissue_length)

    -- Colors
    set_vector_to_data(appearance.colors, appearance_data.colors)

    -- Genes
    set_vector_to_data(appearance.genes.appearance, appearance_data.genes.appearance)
    set_vector_to_data(appearance.genes.colors, appearance_data.genes.colors)
end
...
function import_adv()
    ...
    if unit_data.race ~= target_unit.race or unit_data.sex ~= target_unit.sex then
